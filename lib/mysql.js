...
_.extend(Base, {
  ...
  parseSchema: function () {
    ...
    if (!schema) {
      throw new Error('parseSchema: Missing schema');
    }
...
// Validation Helpers
// ------------------
function missing (value) { return (undefined === value || null === value); }
...
Base.Validators = {
  Required: function (value) {
    ...
    if (missing(value)) throw new ValidationError('REQUIRED');
  },
  Length: function () {
  ...
  Type: {
    ...
    Text: function (value) {
      if (!arguments.length) return arguments.callee;
      if (missing(value)) return;
      if (!_.isString(value)) throw new ValidationError('TYPE-TEXT', value);
    },
...
Base.Validators.Required.when = function (opt) {
  ...
  return function (value, attrs) {
    if (attrs[field] && attrs[field] === fieldValue && missing(value)) {
...
Base.Field = {
  ...
  Number: function (type, opts) {
    if (_.isObject(type)) { return arguments.callee.call(this, opts, type) }
    opts = opts||{};
    type = (type||opts.size||opts.type||opts.length||'').toUpperCase();
    
    var spec = { sql: '', validators : [Base.Validators.Type.Number] }
    var sql, validators = spec.validators;
    ...
    if (opts.unsigned || opts.signed === false) sql += ' UNSIGNED';
    if (opts.signed === true) sql += ' SIGNED';
    if (opts.unique === true) sql += ' UNIQUE';
    if (opts.null === false || opts.required === true ) {
      sql += ' NOT NULL';
      validators.unshift(Base.Validators.Required);
    }
    spec.sql = sql;
    return spec;
  },
  Text: function (size, opts) {  
    if (_.isObject(size)) { return arguments.callee.call(this, opts, size) }
    opts = opts||{};
    size = size||opts.size||opts.length;
    
    var spec = { sql: 'TEXT', validators : [Base.Validators.Type.Text] }
    var sql = spec.sql, validators = spec.validators;
    ...
    if (size) {
      if (isNaN(Number(size))) {
        ...
        sql = type+'('+size+')';
        validators.push(Base.Validators.Length(size));
      }
    }
    ...
    if (parseInt(opts.unique)) {
      spec.keySql = 'UNIQUE KEY (%s('+opts.unique+'))';
    }
    if (opts.null === false || opts.required === true) {
      sql += ' NOT NULL';
      validators.unshift(Base.Validators.Required);
    }
    spec.sql = sql.toUpperCase();
    return spec;
  },
  Enum: function (values, opts) {
    ...
    var spec = { sql: 'ENUM (' + values.map(q).join(', ')+ ')' , validators : [Base.Validators.Type.Enum(values)] }
    return spec;
  },
  Foreign: function () {  },
  Document: function () {  },
